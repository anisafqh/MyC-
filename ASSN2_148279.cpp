...
// Resting Metabolic Rate(RMR) for everu USM staff. This program allows USM staff 
...
void staffInfo(string[], string[], char[], int[],
			   double[], double[], int);
void addStaff();
void displayChoice();
...
void searchStatistic(double[], char[], int[], int);
void seriousCases(double[], int[], char[], int);
void seriousCases2(double[], int[], char[], int);
void displayAdvice(string[], double[], double[], 
				   double[],int);
void outputFile(string[] , string[], char[], int[], double[],
			   double[], int);
...
int main()
{
	...
	char pick, choose;		// To hold choices 	
	...
	char gender[size];		// To hold gender
	...
	if(! staffFile.eof())
		{
			...
			staffFile >> staffID[i] >> gender[i] >> age[i] >> weight[i] >> height[i];
				...
				while(staffFile)
				{
					...
					// display BMI's level of each staff
					if (BMI[i] < 20)
					{
						cout << "\t" << "Underweight" << "\t";
					}
		...
		do
		{
			...
			if (choice != QUIT_CHOICE)
			{
				...
				else if(choice == 5)
				{
					...
					switch(pick)
					{
						case 'A' : rangeBMI20(BMI, size);	// Display number of staff that have BMI less than 20
							break;
						case 'B' : rangeBMI25(BMI, size);	// Display number of staff that have BMI more than 25
							break;
						case 'C' : rangeBMINormal(BMI, size);	// Display number of staff that have normal BMI
							break;
						case 'D' : rangeBMIObese(BMI, size);	// Display number of staff that have BMI more than 30
							break;
...
void staffInfo(string NAME[], string STAFFID[], char GENDER[], int AGE[], double WEIGHT[],
...
void rangeBMINormal(double BMIVAL[], int size)
{
	...
	for (j = 0; j < size; j++)
	{		
		if(BMIVAL[j] >= 20 && BMIVAL[j] < 25)	// BMIVAL less than 25, more than or equal to 20
		{
			sum += BMIVAL[j];	
		}
	}
...
void searchStatistic(double BMIVAL[], char GENDER[], int AGE[], int size)
{
	int age1, j;																
	...
	{		
		if( AGE[j] == age1 )	// if user's age entered same as in the array
		{
			...
			else if (GENDER[j] == 'F')	// if user's gender entered same as in the array
			{
...
void seriousCases(double BMIVAL[], int AGE[], char GENDER[], int size)
{
	double sum = 0;
	int average;
	int j;
	...
	for (j = 0; j < size; j++)
	{		
		if(BMIVAL[j] > 25 && AGE[j] > 20 && GENDER[j] == 'F')
		{
			sum += BMIVAL[j];	
		}
	}
...
void seriousCases2(double BMIVAL[], int AGE[], char GENDER[], int size)
{
	double sum = 0;
	int average;
	int j;
	...
	for (j = 0; j < size; j++)
	{		
		if(BMIVAL[j] > 25 && AGE[j] > 20 && GENDER[j] == 'M')
		{
			sum += BMIVAL[j];	
		}
	}
...
void outputFile(string NAME[], string STAFFID[], char GENDER[], int AGE[], double WEIGHT[],
			   double HEIGHT[], int size)
{
	int i;
	fstream staffFile;
